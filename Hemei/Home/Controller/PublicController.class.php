...
class PublicController extends Controller {
    ...
    public function getCategoryUri($id){
        $info = D('Category')->getCategoryInfo(0,$id);
        $uri = '#';
        switch($info['type']){
            case '0' :
                $uri = U('/Home/Article/index/cid/'.$id);
                break;
            case '1' :
                $uri = $info['uri'];
                break;
    ...
    public function getNewsUri($id){
        $info = D('News')->getNews($id);
        $uri = '#';
        switch($info['type']){
            case '0' :
                $uri = U('/Home/Article/index/cid/'.$id);
